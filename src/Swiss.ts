...
export function Swiss(
  ...
  const scoreGroups = Object.keys(scoreGroupsWithPlayers).sort(
    (s1: string, s2: string) => Number(s2) - Number(s1)
  );
  ...
  if (playerArray.length % 2 > 0) {
    for (let i = scoreGroups.length - 1; i >= 0; i--) {
      ...
      const byeables = players.filter(
        (p) => !p.hasOwnProperty("receivedBye") || !p.receivedBye
      );
